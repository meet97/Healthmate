<%-- 
    ...
    Created on : Feb 28, 2018, 11:15:38 PM
    Author     : sonav
--%>
        ...
        <table>
            ...
            try {
                ...
                if(all.size()>0)
                {
                    ...
                    for (int i = 0; i < all_appointment.size(); i++)
                    {
                        Appointmentdetails add = all_appointment.get(i);
                        Doctordetails dd = add.getDoctorId();
                        Hospitaldetails hd = add.getHospitalId();
                        Logindetails login_details = add.getLoginId();
                        
                        Criteria crr = se.createCriteria(Userdetails.class);
                        crr.add(Restrictions.eq("loginId", login_details));
                        ArrayList<Userdetails> all_userdetails = (ArrayList<Userdetails>)crr.list();
                        Userdetails ud = all_userdetails.get(0);
                        
                        if(add.getFlag())
                        {
                            out.println("<tr>");
                            out.println("<td>"+i+"</td>");
                            out.println("<td>"+ud.getFirstname()+" "+ud.getLastname()+"</td>");
                            out.println("<td>"+hd.getHospitalname()+"</td>");
                            out.println("<td>"+dd.getFirstname()+" "+dd.getLastname()+"</td>");
                            out.println("<td>"+add.getDate()+"</td>");
                            out.println("<td>"+add.getTime()+"</td>");
                            out.println("</tr>");
                        }
                    }
